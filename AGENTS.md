...
- **`paths-ignore` filters the WORKFLOW, not a job.** A gate that has to see documentation-only changes therefore needs its own workflow file; no job added to `ci.yml` can escape that file's `paths-ignore: ["**/*.md", "context/**"]`. `.github/workflows/prompt-ratchet.yml` exists for exactly this reason: every section the review agent's prompt is distilled from lives in `AGENTS.md` or `test-plan.md`, so while the ratchet ran only inside `npm test` it went red **by accident** — on the changes that happened to touch code too — and stayed silent on the docs-only change it was built for. Before adding a gate, check which trigger actually reaches the files it guards.
