...
| H-14 | sentry-monitoring              | dowiedzieć się, że produkcja się wywróciła, zanim powie o tym użytkownik — błędy z Workera trafiają do Sentry, DSN wyłącznie z sekretu              | MVP (S-01…S-06)  | Guardrails: prywatność (żaden DSN w repo), NFR: dostępność produktu       | in progress |
| H-15 | remove-sentry-probe            | przestać wystawiać publiczny endpoint, który celowo wywraca Workera — sonda `/api/shipprobe` znika, gdy monitoring jest już potwierdzony na prodzie | H-14             | Guardrails: prywatność/nadużycie (kwota Sentry), NFR: dostępność produktu | not started |
...
- **Status:** in progress — wiersz założony przez `/10x-implement` (p2), a nie wstecz: `/10x-archive` domknie Status po archiwizacji. Zapisany OD RAZU właśnie dlatego, że mechanizm „zmiana archiwizuje się bez wiersza i znika z roadmapy" wystąpił w tym projekcie już czterokrotnie (H-04, H-07, H-08, H-13) i sam plan tej zmiany go nie przewidywał.

### H-15: Usunięcie sondy błędu z produkcji (post-MVP)

- **Outcome:** (hardening) `src/pages/api/shipprobe.ts` znika z repo i z produkcji, a wraz z nim publiczny, nieuwierzytelniony endpoint, który na każde wejście rzuca wyjątkiem. Do momentu usunięcia jest to jedyny sposób sprowokowania błędu PIERWSZOPLANOWEGO na prodzie — reszta tras odpowiada własnymi komunikatami i nie rzuca — więc kasowanie go ma sens dopiero wtedy, gdy monitoring został tam potwierdzony i nikt nie potrzebuje tego powtarzać.
...
- **Unknowns:** czy przed usunięciem warto zostawić trwały, STRZEŻONY zamiennik (endpoint za nagłówkiem z sekretem albo za `PROTECTED_ROUTES`), żeby dało się prowokować realny błąd na prodzie także później. To osobna decyzja projektowa, nie domyślna część tego zadania.
- **Risk:** Ryzyko nie polega na usunięciu, tylko na ZWLEKANIU. Sonda jest **publiczna i nieuwierzytelniona świadomie** — wariant za guardem logowania został zaproponowany i odrzucony 2026-08-12 na rzecz wygody wywołania `curl`-em z produkcji bez sesji. Każde trafienie to jedno **niesamplowane** zdarzenie (klasa pierwszoplanowa przechodzi w 100%, patrz `DEPENDENCY_NOISE` w `src/worker.ts`), więc pętla na tym adresie wyczerpuje kwotę Sentry — a to jest dokładnie ta awaria samomaskująca, przed którą ostrzega komentarz przy samplowaniu: po przekroczeniu limitu przestają dochodzić także NIEZWIĄZANE błędy i żaden kanał o tym nie powie, bo ten projekt nie ma kanału powiadomień. Jednoliniowy fix pośredni, gdyby szum się pojawił przed usunięciem: dopisać `/api/shipprobe` do `PROTECTED_ROUTES` w `src/middleware.ts`.
- **Status:** not started — wiersz założony 2026-08-12 w trakcie shipu H-14, razem z kodem sondy, a nie wstecz. Ten projekt ma udokumentowane cztery przypadki (H-04, H-07, H-08, H-13), w których zmiana zarchiwizowała się bez wiersza i zniknęła z roadmapy; tu ryzyko jest gorsze niż zniknięcie z dokumentu, bo zapomniany wiersz zostawia na produkcji działający endpoint wywracający Workera.
