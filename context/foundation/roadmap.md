...
| H-17 | bug-generation-deck-undo-swallowed    | dowiedzieć się o pustej talii, którą zostawiło nieudane cofnięcie — zamiast trafić przy „Ponów" na trwałe 409 obwiniające wybraną nazwę                                                          | MVP (S-01…S-06)  | FR-018 (jasny błąd po awarii generacji), US-01                                  | done        |
| H-18 | bug-generation-failed-audit-swallowed | wiedzieć, że nieudany zapis wiersza audytu po awarii generacji nie ginie po cichu — dostaje osobny, wciąż ponawialny komunikat, a właściciel dostaje zgłoszenie do Sentry bez treści użytkownika | MVP (S-01…S-06)  | FR-018 (jasny błąd po awarii generacji), US-01, Guardrails: prywatność          | done        |
| H-19 | bug-signout-swallowed                 | wiedzieć, że wylogowanie NIE poszło i sesja wciąż żyje — zamiast dostać pożegnanie i zostać zalogowanym na wspólnym komputerze                                                                   | MVP (S-01…S-06)  | FR-002 (logowanie i wylogowanie), Guardrails: prywatność per-konto              | done        |
| H-20 | bug-middleware-getuser-swallowed      | usłyszeć, że to BACKEND jest chwilowo nieosiągalny — zamiast „Twoja sesja wygasła" nad sesją, która żyje i wróci za chwilę                                                                       | MVP (S-01…S-06)  | FR-002 (logowanie i wylogowanie), NFR: UI po polsku, Guardrails: prywatność     | done        |
| H-21 | dev-db-test-data-debt                 | (harness) ufać procedurze celowego psucia — spłacony lokalny dług danych testowych i asercje, które nie mogą zzielenieć przez ucięcie okna PostgREST                                             | MVP (S-01…S-06)  | Guardrails: izolacja per-konto (pośrednio — falsyfikowalność jej dowodu)        | in progress |
...
### H-18: Nieudany zapis wiersza audytu przestaje ginąć po cichu (post-MVP)

- **Outcome:** (hardening) Gdy sam zapis wiersza audytu `failed` po nieudanej generacji się nie powiedzie — na obu gałęziach: transportowej `catch` (502) i granicy zero-zapisanych kart (422) — użytkownik nadal dostaje jasny, ponawialny komunikat, tyle że rozszerzony o zdanie informacyjne, że szczegóły błędu nie zostały zapisane (nigdy jako druga sprawa do rozwiązania — jego karty i tak nic nie tracą). Strata wiersza trafia teraz na DWÓCH kanałach zamiast żadnego: osobny literał w odpowiedzi plus jedno `Sentry.captureException` na miejsce, otagowane kodem PostgREST, niosące każdą nieprywatną kolumnę i własne wolnotekstowe pola przyczyny (`message`/`details`/`hint`) jako długość + prefiks SHA-256 zamiast treści — nigdy surowy `PostgrestError`, bo naruszenie CHECK wkłada `Failing row contains (…)`, wklejony tekst włącznie, do DETAIL, które PostgREST przekazuje dalej. Trzeci i ostatni z połkniętych zapisów w `generate.ts` po H-16 (kompensacja) i H-17 (cofnięcie talii) — klasa jest zamknięta w tym pliku; jedynym pozostałym odrzuconym wynikiem zapisu Supabase w `src/` zostaje `src/pages/api/auth/signout.ts:7`, świadomie NIE zabrany tutaj.
- **Change ID:** bug-generation-failed-audit-swallowed
- **PRD refs:** FR-018 (jasny błąd + możliwość ponowienia generacji), US-01, §Guardrails (prywatność tekstu źródłowego — kanał do Sentry nie może przenosić treści)
- **Prerequisites:** MVP (S-01…S-06) — konkretnie S-04 (ścieżka generacji) i S-05 Phase 6 (klucz idempotencji)
- **Parallel with:** — (H-16 i H-17 są już zamknięte i dotyczyły innych gałęzi tego samego pliku)
- **Blockers:** —
- **Unknowns:** **rozstrzygnięte na etapie planu (D-03, D-04, D-05).** Druga gałąź `!data` obok `if (error)` ODRZUCONA: `.single()` nie ma ścieżki cichego zera wierszy (Postgres odpowiada `406/PGRST116`), więc taka gałąź byłaby testem, którego żaden przebieg celowego zepsucia nie potrafi zaczerwienić. Przekazanie surowego `PostgrestError` do Sentry ODRZUCONE — pierwszy argument `captureException` ląduje na zdarzeniu, gdzie żaden builder go nie dosięgnie, więc zamiast tego leci syntetyczny `Error`, a przyczyna jedzie jako sfingerprintowany parametr. Prowokacja realnym DSN-em ODRZUCONA na tym etapie — dowód dostawy jest możliwy wyłącznie na wdrożonym Workerze, a `/api/shipprobe` (jedyny instrument, który to kiedykolwiek pokazał) już nie istnieje (H-15); dług ma teraz właściciela w `follow-ups/sentry-delivery.md`.
- **Risk:** Ryzyko dowodowe, tej samej rodziny co H-16/H-17: żaden przebieg z zestawu testów nie potrafi wymusić awarii samego zapisu wiersza audytu, więc tę połowę dowodzą wyłącznie DWA ręczne przebiegi z jednym `revoke` każdy (a nie jeden, jak u obu poprzedników) — bo obie strony różnią się w pięciu polach i strona zero-zapisanych kart nie jest sterowalna z `.env` (model musi zwrócić karty, które WSZYSTKIE nie przejdą Zod), więc dowiedzenie jej potrzebowało DRUGIEGO dublera modułu, dopuszczonego na własnych warunkach wyjątku z `test-plan.md` §6.9 (nieosiągalne inaczej, tymczasowy, uruchamiany osobno, usunięty, usunięcie udowodnione) i jawnie NIE jako precedens. Drugie ryzyko jest miarą pokrycia, nie luką: przebieg B5 celowo sprawdził, że przywrócenie starego (połkniętego) zachowania na jednej gałęzi zostawia **cały zestaw testów zielony** — to jest granica dowodowa tego ticketu, zmierzona, a nie ukryta.
- **Status:** done

### H-19: Nieudane wylogowanie przestaje udawać sukces (post-MVP)

- **Outcome:** (hardening) Gdy `supabase.auth.signOut()` odda błąd albo rzuci, użytkownik **dowiaduje się, że nadal jest zalogowany** — zamiast dostać przekierowanie na `/`, wrócić odbiciem middleware'u na `/decks` z własnym mailem w nagłówku i nie znaleźć nigdzie ani słowa o powodzie. To hit #4 audytu „swallowed errors" (2026-08-11) i **ostatni odrzucony wynik zapisu Supabase w `src/`** po H-16/H-17/H-18; różni się od całej trójki jedną rzeczą, która decyduje o wadze: tam ginął wiersz audytu, tutaj **zostaje żywa sesja za ekranem, który mówi „do widzenia"** — oba `return { error }` w `_signOut` (`GoTrueClient.js:3184`, `:3195`) stoją NAD jedynym wywołaniem czyszczącym cookie, więc na każdej klasie transportowej, 500 i 429 nic nie jest ani unieważnione, ani wyczyszczone. Trasa dostaje trzy jawne gałęzie zamiast dwóch punktów połknięcia (`await` bez destrukturyzacji oraz `if` bez `else` przy `createClient() === null`), obie awaryjne lądują na `/auth/signin` — jedynej stronie, której middleware nie odbije, kiedy GoTrue jest nieosiągalny, bo `getUser()` na następnym hopie padnie z tego samego powodu i zabrałby parametr razem z przekierowaniem. Kanałów jest dwa: baner z komunikatem nazywającym żywą sesję i obie drogi wyjścia (zamknij przeglądarkę na wspólnym komputerze; zaloguj się ponownie i wyloguj jeszcze raz) plus jedno `Sentry.captureException` bez identyfikatora użytkownika. Przy okazji domknięte dwie ślepe plamki, które ta zmiana sama czyni nośnymi: sweep `?error=` w `form-endpoint-guards.test.ts` (dotąd zakorzeniony w `src/pages/api/decks`, mimo komentarza mówiącego co innego) i strażnik `Sentry.captureException` (dotąd zaszyty na `src/pages/api/generate.ts`) — ten drugi z catch-allem po całym `src/`, więc następne miejsce przechwytu też nie wyląduje bez nadzoru.
- **Change ID:** bug-signout-swallowed
- **PRD refs:** FR-002 (użytkownik może się zalogować i wylogować), §Guardrails (izolacja danych per-konto — na wspólnym komputerze żywa sesja jest właśnie jej naruszeniem), NFR: UI po polsku
- **Prerequisites:** MVP (S-01…S-06) — konkretnie ścieżka auth z S-01 i zamknięty zbiór `AUTH_MESSAGES` z H-03
- **Parallel with:** — (C10X-52, czyli czytelnicza bliźniaczka Swallow B w `middleware.ts`, jest świadomie POZA zakresem; wcześniejsza wersja planu lądowała na `/decks`, gdzie C10X-52 był de facto prerekwizytem, bo zjadał jedyny kanał w dominującej klasie awarii — wybór `/auth/signin` tę zależność usuwa, a nie defekt)
- **Blockers:** —
- **Unknowns:** **rozstrzygnięte na etapie planu i jego review (D-04, D-05, F1, F4).** Lokalne „domknięcie" sesji na siłę ODRZUCONE: `scope: "local"` i tak robi wywołanie sieciowe, a ręczne czyszczenie cookie wymagałoby wewnętrznej nazwy `sb-<host>-auth-token` i jej chunkowania — dokładnie tego, na czym `lessons.md` zabrania polegać; to jest **wykrycie, nie sprzątanie** (kształt z H-17). Lądowanie na `/decks` ODRZUCONE po pomiarze (F1): `middleware.ts` odpytuje `getUser()` na każdym żądaniu, więc w klasie „GoTrue nieosiągalny" baner nigdy by się nie wyrenderował. Kosztem tej decyzji jest przyjęta, wąska klasa fałszywego alarmu z §2.4 — świadomie zaakceptowana, nie zneutralizowana. Otwarte do POMIARU na starcie fazy 2 (F4): czy globalne wylogowanie unieważnia też token dostępu, czy tylko refresh — od tego zależy zarówno kształt asercji, jak i to, czy test w ogóle musi mieć własne konto.
- **Risk:** Ryzyko dowodowe tej samej rodziny co H-16…H-18, tyle że **obie** gałęzie awaryjne są nieosiągalne z zestawu testów (runner prowadzi zdrowy lokalny stack, a `astro:env/server` jest wklejane w czasie transformacji), więc decyzję trzyma czysta funkcja z tablicą prawdy, a to, że trasa jej naprawdę używa, dowodzi jeden ręczny przebieg z martwym portem — i nic go nie powtarza. Drugie ryzyko jest w treści: komunikat jest CAŁYM kanałem dla osoby, której to dotyczy (trzy wyzwalacze wylogowania to natywne POST-y formularza, więc nie ma wyspy ani przycisku „Ponów"), a w tym samym zbiorze zamkniętym stoi obok niego `AUTH_SESSION_MISSING_MESSAGE`, mówiący rzecz DOKŁADNIE ODWROTNĄ — skrócenie go do „Nie udało się wylogować" kasuje jedyne zdanie, które ma znaczenie.
- **Status:** done

### H-20: Awaria backendu auth przestaje udawać wygasłą sesję (post-MVP)

- **Outcome:** (hardening) Gdy `supabase.auth.getUser()` w `src/middleware.ts` odda błąd, użytkownik z **żywą sesją** dowiaduje się, że to **backend uwierzytelniania jest chwilowo nieosiągalny** — zamiast zostać odbitym na `/auth/signin` bez słowa wyjaśnienia albo dostać w wyspie „Twoja sesja wygasła". To hit #5 i **ostatni** audytu „swallowed errors" (2026-08-11) oraz czytelnicza bliźniaczka H-19: tam ginął wynik ZAPISU, tutaj wynik ODCZYTU. Zmierzone przed poprawką: przy martwym porcie Supabase żądanie `GET /decks` z ważnym ciasteczkiem odpowiada **bajtowo tak samo** jak żądanie gościa — ten sam `302`, ten sam `Location`, ten sam `Vary`, zero `?error=`. Kluczowa różnica wobec H-19 i powód, dla którego to **klasyfikator, a nie `if (error)`**: `getUser()` zwraca błąd także zwykłemu wylogowanemu gościowi, i to **przed jakimkolwiek wywołaniem sieciowym**, więc naiwne sprawdzenie zaświeciłoby baner każdemu anonimowemu odwiedzającemu `/`, `/auth/signin` i `/auth/signup`. Podział trzyma czysta funkcja (`src/lib/auth-outcome.ts`) kluczująca na `name` i `code`, nigdy na `status` — bo `AuthSessionMissingError` **fabrykuje** swój status, zwykłe `500` NIE jest w auth-js klasą ponawialną, a zwyczajnie wygasła sesja wraca jako `AuthApiError(400, validation_failed)`, nie jako brak sesji. Odpowiedź idzie w konwencji wołającego: dokument dostaje `302` na `/auth/signin?error=…`, wołający JSON dostaje **`503`** — nie `401`, bo `src/lib/http.ts` nadpisuje treść KAŻDEGO `401` komunikatem „Twoja sesja wygasła", czyli `401` przywróciłby ten defekt innymi drzwiami. Przy okazji zamknięta trzecia, nieowładnięta niczym powierzchnia `?error=` (`src/middleware.ts` trafia do tabeli w `form-endpoint-guards.test.ts`) i złapana gałąź, której nie miał żaden ticket: rzucona wartość nie-`AuthError` była dotąd nieprzechwyconym `500` na KAŻDYM żądaniu.
- **Change ID:** bug-middleware-getuser-swallowed
- **PRD refs:** FR-002 (użytkownik może się zalogować i wylogować — tu: nie zostaje wylogowany pozornie), NFR: UI po polsku, §Guardrails (prywatność per-konto: `locals.user` celowo bez zmian, żeby baner konfiguracji nie odsłonił się anonimowemu gościowi)
- **Prerequisites:** MVP (S-01…S-06) — konkretnie guard `PROTECTED_ROUTES` z F-03/H-02 i zamknięty zbiór `AUTH_MESSAGES` z H-03 (obie użyte stałe już w nim są, więc strona logowania ręczy za nie bez żadnej pracy)
- **Parallel with:** — (H-19 zamknięte wcześniej tego samego dnia; jego wybór lądowania na `/auth/signin` usunął ZALEŻNOŚĆ od tej zmiany, ale nie defekt)
- **Blockers:** —
- **Unknowns:** **rozstrzygnięte na etapie planu (D-01…D-09).** Przechwyt Sentry ODRZUCONY: middleware biegnie na każdym żądaniu, więc pierwszostronny capture jest z konstrukcji niesamplowany i sam się maskuje przy wyczerpaniu kwoty — świadome zerwanie ze wzorcem dwóch kanałów z H-16…H-19, argumentowane, nie odziedziczone. Zmiana `App.Locals` ODRZUCONA (D-04): `locals.user` zostaje `User | null`, co oszczędza 17 konsumentów i zabija zagrożenie ujawnienia w `Layout.astro` — kosztem jest to, że `/` i `Topbar.astro` podczas awarii nadal czytają się jako wylogowany, co zostało **zmierzone**, nie przemilczane. Kierunek gałęzi `default` klasyfikatora jest zakładem: spada na `no-session`, bo pomyłka w drugą stronę to baner dla zwykłego gościa, czyli ten sam defekt odwrócony.
- **Risk:** Ryzyko dowodowe tej samej rodziny co H-16…H-19, w najostrzejszej postaci: **obie** gałęzie awaryjne są nieosiągalne z zestawu testów, więc decyzję trzyma tablica prawdy nad fabrykowanym wejściem, a to, że trasa jej naprawdę używa, dowodzi jeden ręczny przebieg z martwym portem — para przed/po, kontrola jednej zmiennej i kontrola anonimowa. `tests/middleware.test.ts` nie dostaje **żadnego** przypadku na nową gałąź: jego 23 przypadki są dowodem REGRESJI (ścieżka wylogowanego jest bajtowo nietknięta), a nie pokryciem poprawki. Klasy 500 i 429 nie zostały sprowokowane w ogóle — pokrywa je wyłącznie tablica prawdy.
- **Status:** done

### H-21: Spłata długu danych testowych w lokalnej bazie dev (post-MVP)

- **Outcome:** (harness) Deweloper uruchamiający **procedurę celowego psucia** — jedyny instrument, na którym opiera się każde twierdzenie o pokryciu w `test-plan.md` §6.6 — dostaje **czerwone** tam, gdzie strażnik jest wyłączony. Dziś nie dostaje, i to nie jest hipoteza: lokalna baza urosła do ~20 748 talii przy `max_rows = 1000`, a PostgREST ucina każdy wynik na tym oknie, więc asercja w kształcie „obcej talii NIE MA w tym wyniku" przechodzi, kiedy talia po prostu wypadła poza okno. §6.6 zapisuje dokładnie ten przebieg dla `listDueCounts`: neuter czterech polityk przy CAŁKOWICIE wyłączonym strażniku, a asercja zielona. Koszt długu **nie jest** dyskowy ani czasowy (~20 MB, każda filtrowana kolumna zaindeksowana) — stwierdzenie inaczej byłoby fałszem; jedynym uzasadnieniem jest falsyfikowalność. Zmiana robi cztery rzeczy: **utwardza** obie podatne asercje zbiorem odniesienia **lokalnym dla testu** (żadna polityka, którą neuter wyłącza, go nie zasila — wariant „przeczytaj, co B posiada" byłby tym samym defektem odwróconym, bo `listDecks` nie ma predykatu `user_id` i pod `deck_select using (true)` „posiadaniem" staje się cała baza), **spłaca** backlog jedną kaskadą z `auth.users` przez `npm run db:clean` (raport domyślnie, kasowanie dopiero na `-- --yes`), **zamyka** zmierzony flake 3,3 % przez zderzenie tego samego stemu nazwy talii w dwóch plikach — z regułą wyegzekwowaną strażnikiem, nie zapisaną w prozie — i **mierzy** fałszywy pas, dopóki jeszcze istnieje. Siedem talii-artefaktów z ręcznych przebiegów (dwie z nich `test-plan.md` nazywa „the artifact of record") musi przeżyć, dlatego `npx supabase db reset` jest odrzucony, a nie odłożony.
- **Change ID:** dev-db-test-data-debt
- **PRD refs:** §Guardrails (izolacja danych per-konto) — pośrednio: przedmiotem jest wiarygodność DOWODU na tę izolację, nie sama izolacja; kotwica wtórna, jak przy każdym `H-`
- **Prerequisites:** MVP (S-01…S-06) — konkretnie harness z F-03 i teardown e2e z H-12, który zatrzymał PRZYSZŁY przyrost, ale wprost odmówił spłaty istniejącego (`roadmap.md` H-12)
- **Parallel with:** —
- **Blockers:** —
- **Unknowns:** **rozstrzygnięte na etapie planu.** Automatyczny teardown w Vitest ODRZUCONY: `assertAnonKey` nie ma opt-outu, więc test sięga wyłącznie do własnych wierszy pod RLS i do `auth.users` nie dojdzie **z zasady**, a po czerwonym przebiegu skasowałby dowody. Krok higieny w CI ODRZUCONY jako refutacja, nie odłożenie: CI startuje świeży stack na jednorazowym runnerze przy każdym jobie, więc tam nic nigdy nie narasta. `ORDER BY` na obu RPC ODRZUCONY: utwardzenie ląduje w asercjach, więc żadna migracja nie jedzie i bramka driftu C10X-29 zostaje poza sprawą. Defect B (czerwony przebieg CI #66) dostaje **własny ticket** z projektem eksperymentu — prawdopodobna poprawka poszerza politykę retry, czyli zmienia semantykę harnessu.
- **Risk:** Kolejność faz jest **twardym** ograniczeniem, nie preferencją: pomiar fałszywego pasa jest **nietrwały** — daje się wykonać wyłącznie, dopóki ~20 748 talii istnieje, więc utwardzenie i jego pomiar biegną PRZED sprzątaniem, a po spłacie ta liczba nie jest już odtwarzalna. Sama spłata jest jedyną **nieodwracalną** operacją w zmianie, a wszystkie mechanizmy bezpieczeństwa (inwariant „nie-harnessowe liczniki bez zmian" plus niezależny odczyt siedmiu talii) są **post-hoc** — dlatego przed kasowaniem idzie `pg_dump` jako ubezpieczenie od źle zawężonego wzorca, nie jako plan wycofania. Okno neutera to zapis do polityk RLS: §6.6 zapisuje przypadek, w którym restore po cichu nie zadziałał, bo heredoc poszedł do `docker exec` **bez `-i`** — łapie to wyłącznie diff `pg_policies` przed/po. I granica, która zostaje otwarta: `db:clean` jest wywoływane przez człowieka, więc dług wraca w tempie 68 talii na przebieg, a licznika nikt nie pilnuje.
- **Status:** in progress
...
- **H-17: (hardening) Gdy zapis sesji generacji padnie, a kompensujące cofnięcie świeżo utworzonej talii padnie razem z nim, użytkownik **dowiaduje się o pustej talii, która została** — zamiast zobaczyć zwykłe 500, kliknąć „Ponów" i dostać trwałe `409 "Talia o tej nazwie już istnieje"`, czyli komunikat obwiniający jego wybór nazwy i odbierający przy tym sam przycisk. Ostatni połknięty `await` z audytu „swallowed errors" (2026-08-11) po stronie cofnięcia talii: wynik `deleteDeck` jest odczytywany i rozgałęziany na `data`, nie na samo `error` (pod RLS zero wierszy to `{data: null, error: null}`), a gałąź „cofnięcie nie wyszło" odpowiada osobnym komunikatem nazywającym sierotę i **dwie** drogi wyjścia — odśwież i wybierz ją z listy albo zmień nazwę. Świadomie z `retriable: false`: „Ponów" powtarza payload DOSŁOWNIE, więc na tej gałęzi trafi w sierotę i skończy tym samym 409 — flaga oznacza te przypadki, których powtórzenie **z konstrukcji** nie naprawi, a skoro banera nie ma czym kliknąć, to sam komunikat jest jedyną drogą wyjścia. Granica w tym samym oddechu: to jest **wykrycie, nie sprzątanie** — pusta talia przeżywa nieudane cofnięcie niezależnie od tego, jak głośno zostanie zgłoszona (korekta D-01 z H-16).** — Archived 2026-08-13 → `context/archive/2026-08-13-bug-generation-deck-undo-swallowed/`. Lesson: —.
- **H-18: (hardening) Gdy sam zapis wiersza audytu `failed` po nieudanej generacji się nie powiedzie — na obu gałęziach: transportowej `catch` (502) i granicy zero-zapisanych kart (422) — użytkownik nadal dostaje jasny, ponawialny komunikat, tyle że rozszerzony o zdanie informacyjne, że szczegóły błędu nie zostały zapisane (nigdy jako druga sprawa do rozwiązania — jego karty i tak nic nie tracą). Strata wiersza trafia teraz na DWÓCH kanałach zamiast żadnego: osobny literał w odpowiedzi plus jedno `Sentry.captureException` na miejsce, otagowane kodem PostgREST, niosące każdą nieprywatną kolumnę i własne wolnotekstowe pola przyczyny (`message`/`details`/`hint`) jako długość + prefiks SHA-256 zamiast treści — nigdy surowy `PostgrestError`, bo naruszenie CHECK wkłada `Failing row contains (…)`, wklejony tekst włącznie, do DETAIL, które PostgREST przekazuje dalej. Trzeci i ostatni z połkniętych zapisów w `generate.ts` po H-16 (kompensacja) i H-17 (cofnięcie talii) — klasa jest zamknięta w tym pliku; jedynym pozostałym odrzuconym wynikiem zapisu Supabase w `src/` zostaje `src/pages/api/auth/signout.ts:7`, świadomie NIE zabrany tutaj.** — Archived 2026-08-13 → `context/archive/2026-08-13-bug-generation-failed-audit-swallowed/`. Lesson: —.
- **H-19: (hardening) Gdy `supabase.auth.signOut()` odda błąd albo rzuci, użytkownik **dowiaduje się, że nadal jest zalogowany** — zamiast dostać przekierowanie na `/`, wrócić odbiciem middleware'u na `/decks` z własnym mailem w nagłówku i nie znaleźć nigdzie ani słowa o powodzie. To hit #4 audytu „swallowed errors" (2026-08-11) i **ostatni odrzucony wynik zapisu Supabase w `src/`** po H-16/H-17/H-18; różni się od całej trójki jedną rzeczą, która decyduje o wadze: tam ginął wiersz audytu, tutaj **zostaje żywa sesja za ekranem, który mówi „do widzenia"** — oba `return { error }` w `_signOut` (`GoTrueClient.js:3184`, `:3195`) stoją NAD jedynym wywołaniem czyszczącym cookie, więc na każdej klasie transportowej, 500 i 429 nic nie jest ani unieważnione, ani wyczyszczone. Trasa dostaje trzy jawne gałęzie zamiast dwóch punktów połknięcia (`await` bez destrukturyzacji oraz `if` bez `else` przy `createClient() === null`), obie awaryjne lądują na `/auth/signin` — jedynej stronie, której middleware nie odbije, kiedy GoTrue jest nieosiągalny, bo `getUser()` na następnym hopie padnie z tego samego powodu i zabrałby parametr razem z przekierowaniem. Kanałów jest dwa: baner z komunikatem nazywającym żywą sesję i obie drogi wyjścia (zamknij przeglądarkę na wspólnym komputerze; zaloguj się ponownie i wyloguj jeszcze raz) plus jedno `Sentry.captureException` bez identyfikatora użytkownika. Przy okazji domknięte dwie ślepe plamki, które ta zmiana sama czyni nośnymi: sweep `?error=` w `form-endpoint-guards.test.ts` (dotąd zakorzeniony w `src/pages/api/decks`, mimo komentarza mówiącego co innego) i strażnik `Sentry.captureException` (dotąd zaszyty na `src/pages/api/generate.ts`) — ten drugi z catch-allem po całym `src/`, więc następne miejsce przechwytu też nie wyląduje bez nadzoru.** — Archived 2026-08-14 → `context/archive/2026-08-13-bug-signout-swallowed/`. Lesson: —.
- **H-20: (hardening) Gdy `supabase.auth.getUser()` w `src/middleware.ts` odda błąd, użytkownik z **żywą sesją** dowiaduje się, że to **backend uwierzytelniania jest chwilowo nieosiągalny** — zamiast zostać odbitym na `/auth/signin` bez słowa wyjaśnienia albo dostać w wyspie „Twoja sesja wygasła". To hit #5 i **ostatni** audytu „swallowed errors" (2026-08-11) oraz czytelnicza bliźniaczka H-19: tam ginął wynik ZAPISU, tutaj wynik ODCZYTU. Zmierzone przed poprawką: przy martwym porcie Supabase żądanie `GET /decks` z ważnym ciasteczkiem odpowiada **bajtowo tak samo** jak żądanie gościa — ten sam `302`, ten sam `Location`, ten sam `Vary`, zero `?error=`. Kluczowa różnica wobec H-19 i powód, dla którego to **klasyfikator, a nie `if (error)`**: `getUser()` zwraca błąd także zwykłemu wylogowanemu gościowi, i to **przed jakimkolwiek wywołaniem sieciowym**, więc naiwne sprawdzenie zaświeciłoby baner każdemu anonimowemu odwiedzającemu `/`, `/auth/signin` i `/auth/signup`. Podział trzyma czysta funkcja (`src/lib/auth-outcome.ts`) kluczująca na `name` i `code`, nigdy na `status` — bo `AuthSessionMissingError` **fabrykuje** swój status, zwykłe `500` NIE jest w auth-js klasą ponawialną, a zwyczajnie wygasła sesja wraca jako `AuthApiError(400, validation_failed)`, nie jako brak sesji. Odpowiedź idzie w konwencji wołającego: dokument dostaje `302` na `/auth/signin?error=…`, wołający JSON dostaje **`503`** — nie `401`, bo `src/lib/http.ts` nadpisuje treść KAŻDEGO `401` komunikatem „Twoja sesja wygasła", czyli `401` przywróciłby ten defekt innymi drzwiami. Przy okazji zamknięta trzecia, nieowładnięta niczym powierzchnia `?error=` (`src/middleware.ts` trafia do tabeli w `form-endpoint-guards.test.ts`) i złapana gałąź, której nie miał żaden ticket: rzucona wartość nie-`AuthError` była dotąd nieprzechwyconym `500` na KAŻDYM żądaniu.** — Archived 2026-08-14 → `context/archive/2026-08-14-bug-middleware-getuser-swallowed/`. Lesson: —.
