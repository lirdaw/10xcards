...
| H-16 | bug-generation-compensation-swallowed | kliknąć „Ponów" po nieudanym zapisie kart i naprawdę dostać fiszki — klucz, który raz utknął na trwałym 500, sam się odblokowuje przy następnej próbie | MVP (S-01…S-06)  | FR-018 (ponowienie po awarii generacji), US-01, Guardrails: wiarygodność danych | in progress |
...
- **Status:** in progress — wiersz założony przez `/10x-implement` (p5) razem z doc-syncem, a nie wstecz. Plan tej zmiany wyliczał doc-sync jako `test-plan.md` + `change.md` i roadmapy nie przewidywał; wiersz dopisano mimo to, bo bez niego `/10x-archive` nie ma czego domknąć — mechanizm, który w tym projekcie zadziałał już czterokrotnie (H-04, H-07, H-08, H-13). Status domknie `/10x-archive`.
