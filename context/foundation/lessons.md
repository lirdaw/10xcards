...
- **Rule**: (1) Zanim dopniesz bramkę, sprawdź, czy jej WYZWALACZ sięga plików, których pilnuje. `paths-ignore` filtruje cały workflow, nie job, więc bramka na pliki `.md` potrzebuje własnego pliku workflow; przy `pull_request` filtr liczy się względem całego diffa PR-a. Próbę czerwieni rób NA TEJ ŚCIEŻCE, na której bramka będzie żyła — nie lokalnie. (2) KAŻDY nowy plik workflow dostaje `defaults: run: shell: bash` na starcie. Nie „pamiętaj o `pipefail` przy rurach": domyślny shell w pliku to jedno miejsce zamiast każdej rury z osobna, a pamiętanie o `pipefail` w momencie pisania rury jest dokładnie tym, czego się wtedy nie ma. (3) Para dowodowa jest dowodem TYLKO wtedy, gdy zmienna różnicy NIE MOŻE po cichu zniknąć. Fallback na wartość domyślną w miejscu, które para porównuje, unieważnia parę — a wygląda przy tym jak odporność. Podstawa: przy dowodzeniu strażnika `maxBudgetUsd` literówka w `REVIEW_MAX_BUDGET_USD` zwijająca się do wartości produkcyjnej sprawiłaby, że przebieg „budżet 0.01" pojechałby na limicie 1,00 i skończył zielono; para pokazałaby zieleń w OBU przebiegach i czytałaby się jako „limit nie działa" zamiast „limitu nie podano". Rozwiązane odmową na złą wartość (`abc`, `0`, `-1` → kod 1), nie fallbackiem. Wartość zmiennej różnicy drukuj PRZED wywołaniem, na stderr — nie w linii metryk, bo metryki drukują się tylko na ścieżce sukcesu, a przebieg zatrzymany strażnikiem tam nie dochodzi, więc para nie zostawia w logu żadnego śladu, czym się różniła.
- **Applies to**: plan, implement, impl-review

## Stan niesiony przez DWA pola czyta się z DWÓCH, nie z jednego

- **Context**: Każda gałąź oparta na pojedynczym polu wyniku API, które opisuje ten sam moment co pole sąsiednie. W tym repo zmierzone cztery wystąpienia: `subtype` zamiast `subtype` + `is_error` (wynik Agent SDK, `agents/review/run-review.ts`); `status` zamiast `outcome` (kroki GitHub Actions, `.github/actions/review-agent/action.yml`); `result.error` zamiast `response.error` (promptfoo — pierwsze zlewa awarię providera z pęknięciem asercji, `agents/review/evals/report.ts`); `child.status` zamiast `child.status` + `child.error` (`spawnSync`, `agents/review/evals/report.ts`). Fazy plan / implement / impl-review.
- **Problem**: Objaw jest zawsze ten sam i dlatego trudny do zauważenia: kod działa **fail-closed**, więc NIC NIE PRZECIEKA — a komunikat opisuje NIE TĘ przyczynę. `spawnSync`: proces, który się nie uruchomił (ENOENT), proces ubity limitem czasu (ETIMEDOUT) i przebieg z czerwoną asercją dają ten sam `exitCode = 1`, bo `status` jest `null` w dwóch pierwszych; jedynym komunikatem był „nie udało się wczytać wyniku", czyli diagnoza PLIKU zamiast diagnozy przyczyny. promptfoo: `result.error` przy pękniętej asercji niesie jej treść, więc poprawnie oceniona komórka była klasyfikowana jako brak wyniku — zmierzone na pierwszym pełnym przejściu macierzy, w kodzie, który miał przed tym zlaniem bronić. SDK: `subtype: "success"` przychodzi RAZEM z `is_error: true` przy awarii łączności, i agent raportował „niepoprawny structured output" zamiast awarii dostawcy. Diagnoza kieruje w złą stronę dokładnie wtedy, gdy jest najbardziej potrzebna, a testy tego nie widzą, bo werdykt jest poprawny.
- **Rule**: Zanim oprzesz gałąź na jednym polu, **sprawdź, czy API nie ma drugiego, które opisuje ten sam moment** — jeśli ma, czytaj OBA. Sukces rozstrzygaj na koniunkcji, a przyczynę awarii bierz z pola, które ją NAZYWA, nie z tego, które mówi tylko „nie zero". Diagnozę wynieś do czystej funkcji (wzorzec: `describeSpawnFailure` w `agents/review/evals/report.ts`) i przetestuj każdą klasę osobno, z asercją `doesNotMatch` na cudzą klasę — inaczej „jakiś komunikat jest" przechodzi za komunikat właściwy. Przy polach cudzej biblioteki sprawdź też, czy pole, które czytasz, nie jest współdzielone przez dwa różne zdarzenia (`result.error` w promptfoo niesie i awarię providera, i powód pęknięcia asercji); jeśli jest — czytaj to, które ustawia TWÓJ kod.
- **Applies to**: plan, implement, impl-review

## Liczba czytana w chwili decyzji musi być tą wielkością, o którą decyzja pyta

- **Context**: Każdy komunikat bramki albo CLI, na podstawie którego człowiek decyduje, czy wydać pieniądze, powtórzyć przebieg albo eskalować. W tym repo: `PAID_REMEDY` w `agents/review/evals/eval-record.ts`, drukowane przez workflow `Eval ratchet`. Fazy plan / implement / impl-review.
- **Problem**: `COLD_PASS_COST_ANCHOR_USD = 0.12` niosło **dwie wielkości pod jedną nazwą**: sumę kosztów z raportu przejścia (`costUsd` po komórkach) i rachunek (delta odczytów `/api/v1/key`). Różnią się o komórki, które ZAPŁACIŁY i nie oddały licznika — `error_max_turns` nie zwraca `usage`, więc do sumy nie wchodzi wcale. 0,12 było **prawdziwe jako suma z raportu i fałszywe jako rachunek**, a remedium czyta się dokładnie w chwili decyzji „czy zapłacić", czyli w jedynym kontekście, w którym liczy się rachunek. Zmierzone: dwa realne przejścia kosztowały 0,139255 i 0,235012 USD, a `AGENTS.md` dopisane w TEJ SAMEJ zmianie podawało ~0,24 — dwie powierzchnie jednego repo rozjechane 2× o tej samej komendzie.
- **Rule**: Zanim wstawisz liczbę do komunikatu, na którym ktoś oprze decyzję, **nazwij w identyfikatorze, KTÓRA to wielkość**, nie samą jednostkę. Jeśli dwie wielkości da się pomylić, dostają dwie nazwy, a do komunikatu idzie ta, o którą decyzja pyta. Przy decyzji o wydatku bierz **wyższą** ze zmierzonych, nie średnią — niedoszacowanie kosztuje więcej niż przeszacowanie. Gdy rozrzut jest zmierzony, cytuj **ZAKRES, nie punkt**: pojedyncza liczba obiecuje precyzję, której pomiar nie ma. Wszystkie powierzchnie (kod, `AGENTS.md`, dokumentacja) synchronizuj w TEJ SAMEJ zmianie — rozjazd między nimi jest sam w sobie defektem.
- **Applies to**: plan, implement, impl-review

## Rozjazd między gwarancją a jej opisem idzie w stronę pochlebną — jednorazowa obserwacja czerwieni nie jest kontrolą pozytywną

- **Context**: Bramki, których rdzeń rozstrzyga między kilkoma NAZWANYMI rodzajami awarii — i zapisy, które opisują, co te bramki gwarantują. W tym repo: dziewięć rodzajów w `checkRecord` (`agents/review/evals/eval-record.ts`), dwie odmowy w `scripts/check-verdict-config.ts` oraz `verification.md` / `plan.md`, które je opisują. Fazy plan / implement / impl-review, a także każde czytanie cudzej deklaracji pokrycia.
- **Problem**: Gwarancja i jej OPIS rozjeżdżają się osobno, a w jednej zmianie zdarzyły się obie instancje tego samego. **(1) Obserwacja udaje pokrycie, którego testy nie mają:** pięć z dziewięciu gałęzi czerwieni nie miało żadnej asercji, a trzy z nich widziano na czerwono naprawdę — `callFingerprint` na żywym CI (sonda P1), `missing` w weryfikacji ręcznej 4.8, `malformed` na rekordzie sprzed D-6 — plus dwie odpalone ręcznie w impl-review. Wszystkie działały. Ale obserwacja świeci RAZ i nie zaświeci się przy następnej refaktoryzacji funkcji decydującej; sześć jednorazowych wypatrzeń czyta się jak sześć asercji i nią nie jest. **(2) Dokumentacja udaje wzorzec, którego kod nie niesie:** `verification.md` twierdziło „czternaście nowych przypadków, każdy dwustronny (wzorzec `blindTo`)", podczas gdy `blindTo` z `cache.test.ts` mutuje FUNKCJĘ decydującą, a te testy mutują WEJŚCIE. **Asymetria jest tu treścią, nie ozdobą: OBA rozjazdy poszły w stronę POCHLEBNĄ dla autora.** Nikt nie zaniża własnego pokrycia przez pomyłkę — to jest kształt tego błędu, nie zbieg okoliczności.
- **Rule**: Każdy NAZWANY rodzaj awarii bramki dostaje **własną asercję**; sonda na żywym CI dowodzi okablowania raz, test dowodzi go za każdym razem. Gdy wyjście SAMO NAZYWA oś (`problems[].kind`), mutacja wejścia jest tak samo mocna jak oślepienie funkcji — blindness-testing zarabia na swój szew w kodzie produkcyjnym wyłącznie przy wyjściu NIEPRZEZROCZYSTYM (hash); nie kopiuj wzorca z pamięci, ale ZAPISZ, którego użyłeś. Opis gwarancji trzymaj zsynchronizowany z gwarancją: zapis nazywający dowód mocniejszym, niż jest, to ta sama klasa błędu, którą bramka ma łapać. **Operacyjnie, przy audycie: sprawdzaj deklaracje POKRYCIA, nie deklaracje LUK.** Deklaracja luki jest autokrytyką i prawie zawsze jest prawdziwa; deklaracja pokrycia jest autoreklamą i wymaga sprawdzenia. Tak dziel czas audytu.
- **Applies to**: plan, implement, impl-review

## Proza przenoszona przez zapis potrzebuje własnego warunku nieświeżości

- **Context**: Każdy generowany plik danych, w którym zapisywacz ZACHOWUJE blok pisany ręcznie przez człowieka. W tym repo: `notes` w `agents/review/evals/eval-record.json`, przenoszone przez `--record` ORAZ przez `run-verdict-config.ts --write`. Fazy plan / implement / impl-review.
