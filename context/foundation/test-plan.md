...
|---|-----------|------------------|---------------|------------|--------|----------------|
| 1 | Harness + per-account isolation | Stand up the runner and prove cross-account denial on read and write | #1 | runner bootstrap, integration, RLS | change opened | `context/changes/verification-harness/` |
...
- Phase 1 corresponds to roadmap **F-03 `verification-harness`** (runner +
  this plan + one real cross-account test). It reuses that change-id rather
...
|-------|------|---------|-------|
| unit + integration | Vitest | none yet — see Phase 1 | Astro's own testing guide points at Vitest configured through `getViteConfig()` from `astro/config`; checked: 2026-07-15 |
...
| database under test | Supabase CLI local stack | 2.23.4 (devDependency) | Already used for `db:types`; RLS is only meaningful against a real Postgres |
...
| unit + integration | local + CI | required after §3 Phase 1 | logic regressions, cross-account access, endpoint contract breaks |
...
- TBD — see §3 Phase 1 (runner bootstrap). Phase 4 extends this with the
  rating→next-review mapping pattern.

### 6.2 Adding an integration test

- TBD — see §3 Phase 1 for the cross-account denial pattern (deny on read
  and write, with a positive control on the owner's own data).
...
- TBD — see §3 Phase 1 for the RLS pattern (role plus JWT claims, never as
  `postgres`, always with a positive control).
