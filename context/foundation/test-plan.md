...
>
> Last updated: 2026-08-02 (C10X-42 `eval-ci-dispatch` — not a §3 rollout phase). **No risk row
...
> defect that entry itself catches against C10X-39. Ship-time
> evidence (registration, a green dispatch, a controlled red, and the re-run that settles artifact
> immutability) is **PENDING Phase 4** and is not claimed here. Evidence:
...
- **Roadmap C10X-42 (`eval-ci-dispatch`, 2026-08-02)** — not a §3 rollout phase, and **not a
  ...
  | Registration, both directions of the dispatch, and artifact immutability        | **PENDING Phase 4** — a `workflow_dispatch` workflow is undispatchable until it reaches the default branch, measured on this repository once already (C10X-29: `HTTP 404: workflow schema-diff.yml not found on the default branch`, with the recorded conclusion that there is no honest workaround). Run identifiers, wall clock and observed cost land here and in the change's `verification.md` once the merge has happened                                                                                                                                                                                                                                                                                                                                                                         |
...
- **The blast-radius cap is MEASURED, not assumed — and that is the one claim about this change
  ...
  still a claim only Phase 4's green dispatch can settle, and this entry does not make it.
- **Everything dispatch-related is PENDING Phase 4 and none of it is claimed here.** Registration
  (`gh workflow list` before/after the merge — C10X-29's check, whose "before" half was recorded
  in Phase 2), a green dispatch, a controlled red dispatch on a bogus `generator_model`, and the
  re-run that settles artifact immutability. Run identifiers, wall clock and observed cost land in
  the change's `verification.md` and in the fields §6.6's entry marks PENDING. The reason this
  cannot be pulled earlier is measured rather than assumed: a `workflow_dispatch` workflow is not
  dispatchable until it exists on the default branch — C10X-29 measured it, and the error string
  it recorded is `HTTP 404: workflow schema-diff.yml not found on the default branch`, with the
  written conclusion that there is no honest workaround.
