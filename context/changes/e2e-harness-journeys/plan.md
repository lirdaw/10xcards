...
- [ ] 1.1 `npm run typecheck` exits 0, `Result (N files)` recorded as observed
- [ ] 1.2 `npm run lint` exits 0, pre-existing warning count unchanged
- [ ] 1.3 `npm test` green (also the neutrality check for the `preflight.ts` extraction); `tests/lib/e2e-env.test.ts` passes; suite total recorded as observed
- [ ] 1.4 `npm run e2e` starts its own dev server and completes with none running by hand; `--list` shows the `setup` project collecting a non-zero number of tests
- [ ] 1.5 Breakage: cloud `SUPABASE_URL` → config throws before any server starts; restored
- [ ] 1.6 Breakage: hand-started server on 4321 → Playwright's "already used" hard error
- [ ] 1.7 Breakage: chromium binary hidden → the `npx playwright install chromium` message; restored
- [ ] 1.8 Breakage: cloud `SUPABASE_URL` in a `.dev.vars` over a valid `.env` → throws before any server starts, message names `.dev.vars`; file deleted, green

#### Manual

- [ ] 1.9 `PROD_` swap exercised by hand: refusal names the host, no request reached it, swap back green
- [ ] 1.10 Each refusal message read as a developer would — names the fix, does not diagnose the wrong file
