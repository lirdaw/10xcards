...
- **Decision**: **FIXED (structurally) — the matrix still needs re-running.** `verification.md` gains **§3b**, a seven-row table keyed to criteria 4.5–4.11 with its Observed column deliberately left `_to fill_`, and §4's circular sentence is repointed at it and now states plainly that until someone re-runs it those seven criteria are checked off against no on-disk evidence. Two things are recorded there rather than left implicit: this is the only evidence those criteria can ever have (no automated layer reaches an island, test-plan §7), and 4.7 must be driven against a name-taken 409 rather than the fourth 409 at `:368`, which is deliberately unflagged and therefore still offers "Ponów". **Action for the author: re-run the matrix and fill the column.**
