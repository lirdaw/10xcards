...
- [x] 5.4 Deliberate-breakage check: a scratch migration adding a column makes the job report a difference; revert — run 30381750723 dispatched from `tmp-schema-diff-probe`: job **failure**, diff step **failure**, `Upload the diff for triage` **success** (that path had been skipped twice and never run before). Log carried `4 lines of DDL; the body is in the 'schema-diff' artifact.`, the artifact carried `alter table "public"."deck" drop column "ship_probe_tmp";`, and the body was NOT in the log — which also proves the impl-review's F3 fix. Paired against the two green `main` dispatches, one variable. Branch deleted, 10 migrations, tree clean — 02c1a36
